...
This is how piping works. First, we define the dataset (spin) and then we filter for only Males using the `filter()` function. Note that the `%>%` operator can be read as "then". [@cr-code3]{hlz="1"}
